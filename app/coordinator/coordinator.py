...
async def connect_rabbitmq():
    ...
    connection = await aio_pika.connect_robust(
        rabbit_url, ssl_context=ssl_ctx
    )
...
async def coordinator_main(session_uid: str, coordinator_uid: str):
    ...
    if len(hyperparams_list) < len(peers):
        raise RuntimeError("Not enough hyperparameter configs")

    dataset = session.get("dataset", {})
    csv_link = f"s3://{dataset.get('s3_bucket')}/{dataset.get('s3_key')}"
    x_labels = [c for c in dataset.get("columns", []) if c != "label"]
    y_label = "label"
