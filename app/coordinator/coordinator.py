...
async def coordinator_main(session_uid: str, coordinator_uid: str):
    ...
    # Start consumer BEFORE sending any commands
    ...
    try:
        ...
        # Save final session results nested by UID
        final_results = {}
        for peer in peers:
            results = sessions_collection.find_one(
                {"_id": session_oid}, {"peers": {"$elemMatch": {"uid": peer["uid"]}}}
            ).get("peers", [])
            final_results[peer["uid"]] = {
                "hyperparameters": hyperparams,
                "results": results[0].get("results", []) if results else [],
            }

        sessions_collection.update_one(
            {"_id": session_oid},
            {"$set": {"status": "COMPLETED", "completed_at": datetime.utcnow(), "coordinator_uid": coordinator_uid, "final_results": final_results}},
        )
        users_collection.update_one(
        ...
        print(f"[coordinator:{coordinator_uid}] session COMPLETED and coordinator reset to OFFLINE", flush=True)
